...
def test_liouvillian(kT, bosons, rate):
    ...
    basis = H.eigh()
    bin_width = 1e-6
    c_opsp, c_opsm = no.collapses(c, basis, kT, "fermionic", rate, bin_width)
    c_ops = [c_opsp, c_opsm]
...
def test_spectrum():
    ...
    I8 = nme.spectrum_resolvent(L, a, frequency, diagonalize=True)
...
def test_g2():
    ...
    basis = H.eigh()
    bin_width = 1e-6
    ap, am = no.collapses(a, basis, 0.1, "bosonic", 0.1, bin_width)
