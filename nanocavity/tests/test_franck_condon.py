...
def test_franck_condon():
    ...
    for g in g_vec:
        assert np.allclose(FC_0n(0, g) ** 2, nfc.FC_factor(0, 0, g))
        assert np.allclose(FC_0n(1, g) ** 2, nfc.FC_factor(0, 1, g))
        assert np.allclose(FC_11(g) ** 2, nfc.FC_factor(1, 1, g))
        assert np.allclose(FC_21(g) ** 2, nfc.FC_factor(2, 1, g))
        assert np.allclose(FC_22(g) ** 2, nfc.FC_factor(2, 2, g))
