...
def test_bath_system_bath_rate():
    ...
    Gup = kappa * ndist.bose_einstein(omegac, kT) + m * (ndist.Fermi_cb(bias-E, kT) + ndist.Fermi_cb(-bias-E, kT))
    Gdw = kappa * (1 + ndist.bose_einstein(omegac, kT)) + m * (ndist.Fermi_cb(bias+E, kT) + ndist.Fermi_cb(-bias+E, kT))
    x = Gup / Gdw
